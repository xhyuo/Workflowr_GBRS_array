---
title: "Figure3_transeqtl_FDR0.01"
author: "Hao He"
date: "`r Sys.Date()`"
output:
  ...
  toc: true
vignette: >
  %\VignetteIndexEntry{Figure3_transeqtl_FDR0.01}
...
# Figure 3 Trans-eQTL at FDR 0.01 (5 Mb)
```{r Figure 3 Trans-eQTL at FDR 0.01 5Mb, message=FALSE, eval=TRUE,fig.height=16, fig.width=16}
library(sinaplot)
library(ggplot2)
library(plyr)
library(VennDiagram)
...
a <- a[a$max_qvalue <= 0.01 & a$cis_trans == "trans",]
...
b <- b[b$max_qvalue <= 0.01 & b$cis_trans == "trans",]
...
c <- c[c$max_qvalue <= 0.01 & c$cis_trans == "trans",]
...
barplot(c(a3[[1]],a3[[2]],a3[[3]]),
        ylab = "Number of trans-eQTL at FDR 0.01",
...
# Figure 3 Scatterplot for Trans-eQTL at FDR 0.01 (5 Mb)
```{r Figure 3 Scatterplot for Trans-eQTL at FDR 0.01 5Mb, message=FALSE, eval=TRUE,fig.height=16, fig.width=16}
load("/projects/heh/HH/GBRS_array/data/gene.anno.cis.RData")
...
b0 <- eqtl[, bc_pair$genes]
b0 <- b0[unique(bc_pair$eqtl_marker), ]
b0 <- setNames(melt(b0), c('eqtl_marker', 'genes', 'lod'))
b0$eqtl_marker <- as.character(b0$eqtl_marker)
b0$genes <- as.character(b0$genes)
bc_pair_b_lod <- merge(b0, bc_pair, by = c("eqtl_marker", "genes"), all.y = TRUE)
...
c0 <- eqtl[, bc_pair$genes]
c0 <- c0[unique(bc_pair$eqtl_marker), ]
c0 <- setNames(melt(c0), c('eqtl_marker', 'genes', 'lod'))
c0$eqtl_marker <- as.character(c0$eqtl_marker)
c0$genes <- as.character(c0$genes)
bc_pair_c_lod <- merge(c0, bc_pair, by = c("eqtl_marker", "genes"), all.y = TRUE)
bc_pair_c_lod$g <- "FIXED.MUGA"

bc_pair_lod <- merge(bc_pair_b_lod, bc_pair_c_lod, by = c("eqtl_marker", "genes"))
bc_pair_lod$g <- ifelse(bc_pair_lod$lod.x >= b_cutoff & bc_pair_lod$lod.y >= c_cutoff, "Both", ifelse(bc_pair_lod$lod.x >= b_cutoff & bc_pair_lod$lod.y < c_cutoff,"GBRS","FIXED.MUGA"))
...
bc_pair_lod <- merge(bc_pair_b_lod, bc_pair_c_lod, by = c("eqtl_marker", "genes"))
bc_pair_lod$g <- ifelse(bc_pair_lod$lod.x >=b_cutoff & bc_pair_lod$lod.y >= c_cutoff, "Both", ifelse(bc_pair_lod$lod.x >= b_cutoff & bc_pair_lod$lod.y < c_cutoff,"GBRS","FIXED.MEGAMUGA"))
...
bc_pair_lod <- merge(bc_pair_b_lod, bc_pair_c_lod, by = c("eqtl_marker", "genes"))
bc_pair_lod$g <- ifelse(bc_pair_lod$lod.x >= b_cutoff & bc_pair_lod$lod.y >= c_cutoff, "Both", ifelse(bc_pair_lod$lod.x >= b_cutoff & bc_pair_lod$lod.y < c_cutoff,"GBRS","FIXED.GIGAMUGA"))
